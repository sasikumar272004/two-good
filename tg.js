 ...
 function videoconAnimation() {
  ...
  videocon.addEventListener('mousemove', (e) => {
    ...
    gsap.to(playbtn, {
      ...
      top: e.clientY - rect.top - 30, // Adjust for play button position if needed
...
function cursorAnimation() {
  ...
  document.querySelectorAll(".child").forEach(child => {
    child.addEventListener("mouseenter", () => {
      gsap.to(cursor, {
        zIndex: '9999', // Ensuring the cursor is above other elements but not interfering
    ...
    child.addEventListener("mouseleave", (e) => {
      ...
      if (!(cursorX >= rect.left && cursorX <= rect.right && cursorY >= rect.top && cursorY <= rect.bottom)) {
        gsap.to(cursor, {
          transform: 'translate(-50%,-50%) scale(0)', // Scaling back to normal size instead of disappearing
          duration: 0.3,
          zIndex: '999', // Adjusting zIndex back so it doesn't interfere
        });
      }
    });
  });
  ...
  document.addEventListener("scroll", () => {
    ...
    if (!isCursorInsideChild) {
      gsap.to(cursor, {
        transform: 'translate(-50%,-50%) scale(1)', // Scaling back to normal size instead of disappearing
        duration: 0.3,
        zIndex: '999', // Adjusting zIndex back so it doesn't interfere
      });
    }
